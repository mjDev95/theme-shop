	...
	<table cellspacing="0" class="shop_table shop_table_responsive">
		<tr class="cart-subtotal">
			...
			<td data-title="<?php esc_attr_e( 'Subtotal', 'woocommerce' ); ?>">
				<?php echo WC()->cart->get_cart_subtotal() . ' <span class="text-muted">' . $currency . '</span>'; ?>
			</td>
		</tr>
			...
			<?php foreach ( WC()->cart->get_tax_totals() as $code => $tax ) : ?>
				<tr class="tax-rate tax-rate-<?php echo esc_attr( sanitize_title( $code ) ); ?>">
					...
					<td data-title="<?php echo esc_attr( $tax->label ); ?>">
						<?php echo wp_kses_post( $tax->formatted_amount ) . ' <span class="text-muted">' . $currency . '</span>'; ?>
					</td>
				</tr>
			<?php endforeach; ?>
		<?php endif; ?>
		...
		<tr class="order-total">
			...
			<td data-title="<?php esc_attr_e( 'Total', 'woocommerce' ); ?>">
				<strong><?php wc_cart_totals_order_total_html(); ?> <span class="text-muted"><?php echo $currency; ?></span></strong>
			</td>
		</tr>
